...
export const getPreviousDelay = (delays: number[], index: number) => (index > 0 ? delays[index - 1] : 0);

export const accumulateDelays = (accumulatedDelays: number[], action: Action, index: number, delayRange?: Range) => [
  ...accumulatedDelays,
...
export const getDelays = (actions: Action[], delayRange?: Range) =>
  actions.reduce<number[]>(
    (accumulatedDelays, action, index) => accumulateDelays(accumulatedDelays, action, index, delayRange),
    [],
  );

export const startTimers = (actions: Action[], then: () => void) => (delay: number, index: number) =>
